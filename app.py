...
class LogEntry:
    def __init__(self, app, data):
        ...
        self.key = data["key"]
        self.open = data["open"]
        self.created_at = dateutil.parser.parse(data["created_at"]).astimezone(
            timezone.utc
        )
        ...
        self.closed_at = (
            dateutil.parser.parse(data["closed_at"]).astimezone(timezone.utc)
            if not self.open
            else None
        )
...
class User:
    ...
    def __str__(self):
        if self.discriminator == "0" or self.discriminator == "0000":
            return self.name
...
class Message:
    def __init__(self, data):
        ...
        self.created_at = dateutil.parser.parse(data["timestamp"]).astimezone(
            timezone.utc
        )
        self.human_created_at = duration(
            self.created_at, now=datetime.now(timezone.utc)
