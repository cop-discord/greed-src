...
class Voicemaster(Cog):
    ...
    async def voicemaster(self, ctx: GreedContext):
        """
    ...
    @is_vm()
    async def vm_setup(self, ctx: GreedContext):
        """
        ...
        async with self.locks[ctx.guild.id]:
            ...
            category = await ctx.guild.create_category(
                name="vm", reason="voicemaster category created"
            )
    ...
    @has_guild_permissions(administrator=True)
    @bot_has_guild_permissions(manage_channels=True)
    async def vm_unsetup(self, ctx: GreedContext):
        """
    ...
    @has_guild_permissions(administrator=True)
    async def interface(self, ctx: GreedContext, *, code: str = None):
        """
        ...
        if not code:
            embed = (
                Embed(
                    ...
                    description="Control the voice channels created by the bot",
                )
                .set_author(name=ctx.guild.name, icon_url=ctx.guild.icon)
                .set_thumbnail(url=self.bot.user.display_avatar.url)
                .set_footer(text="greed")
                .add_field(
                    name="usage",
                    value="\n".join(f"{x[0]} - {x[1]}" for x in self.values),
                )
    ...
    @check_vc_owner()
    @bot_has_guild_permissions(manage_channels=True)
    async def lock(self, ctx: GreedContext):
        """
        Lock the voice channel
        """
    ...
    @check_vc_owner()
    @bot_has_guild_permissions(manage_channels=True)
    async def unlock(self, ctx: GreedContext):
        """
        Unlock the voice channel
        """
    ...
    @rename_cooldown()
    @bot_has_guild_permissions(manage_channels=True)
    async def rename(self, ctx: GreedContext, *, name: str):
        """
        Rename the voice channel
        """
    ...
    @check_vc_owner()
    @bot_has_guild_permissions(manage_channels=True)
    async def hide(self, ctx: GreedContext):
        """
        Hide the voice channel
        """
    ...
    @check_vc_owner()
    @bot_has_guild_permissions(manage_channels=True)
    async def reveal(self, ctx: GreedContext):
        """
        Reveal the voice channel
        """
    ...
    @check_vc_owner()
    @bot_has_guild_permissions(manage_channels=True)
    async def permit(self, ctx: GreedContext, *, member: Member):
        """
    ...
    @check_vc_owner()
    @bot_has_guild_permissions(manage_channels=True)
    async def reject(self, ctx: GreedContext, *, member: Member):
        """
    ...
    @check_vc_owner()
    @bot_has_guild_permissions(manage_channels=True)
    async def kick(self, ctx: GreedContext, *, member: Member):
        """
    ...
    async def claim(self, ctx: GreedContext):
        """
    ...
    @voice.command(brief="vc owner")
    @check_vc_owner()
    async def transfer(self, ctx: GreedContext, *, member: Member):
        """
