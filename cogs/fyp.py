    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "http://undefined.rip/api/1.0/tiktok/EN"
            ) as feed_request:
...
class poster(commands.Cog):
    ...
    async def fyp(self, ctx):
        async with ctx.typing():
            ...
            file = discord.File(
                io.BytesIO(downloaded_data), filename=f"{self.bot.user.name}.mp4"
            )
            ...
            await ctx.reply(
                embed=discord.Embed(
                    title="FYP Tiktok Video", color=self.bot.color
                ).set_footer(
                    text=f"💬 {comments} | ❤️ {likes} | 🔗 {shares} | 👤 {ctx.author}",
                    icon_url="https://media.discordapp.net/attachments/1136728813674430528/1143966118868963388/multi_coloured_star.png",
                ),
                file=file,
