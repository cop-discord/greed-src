...
class Boosterrole(Cog):
    ...
    # changed fetchrow to fetch to validate multiple queries instead of 1
    ...
    async def br_setup(self, ctx: GreedContext):
        """
    ...
    @has_guild_permissions(manage_guild=True)
    @br_is_configured()
    async def br_reset(self, ctx: GreedContext):
        """
        ...
        async def no_callback(interaction: Interaction):
            return await interaction.response.edit_message(
                embed=Embed(
                    color=self.bot.yes_color,
                    description="Alright you changed your mind",
                ),
                view=None,
            )
    ...
    @has_guild_permissions(manage_guild=True)
    async def br_award_add(self, ctx: GreedContext, *, role: NewRoleConverter):
        """
    ...
    @has_guild_permissions(manage_guild=True)
    async def br_award_remove(self, ctx: GreedContext, *, role: NewRoleConverter):
        """
    ...
    async def br_award_list(self, ctx: GreedContext):
        """
    ...
    @has_guild_permissions(manage_guild=True)
    @br_is_configured()
    async def br_base(self, ctx: GreedContext, *, role: Role = None):
        """
    ...
    @br_is_configured()
    async def br_create(self, ctx: GreedContext, *, name: str = None):
        """
      ...
      """,
        ...
        await self.bot.db.execute(
            """
            INSERT INTO shared_roles (guild_id, role_id, user_id, owner_id) 
            VALUES ($1, $2, $3, $4)
            """,
            ctx.guild.id,
            role.id,
    ...
    @has_br_or_shared_role()
    async def br_name(self, ctx: GreedContext, *, name: str):
        """
        ...
        if not role:
            return await ctx.send_warning(
                f"You don't have a booster role or a shared booster role\nPlease ask a server booster to share their role with you or create a new booster role"
            )
    ...
    @has_br_or_shared_role()
    async def br_color(self, ctx: GreedContext, *, color: HexColor):
        """
    ...
    @has_br_or_shared_role()
    @boosted_to(2)
    async def br_icon(self, ctx: GreedContext, *, emoji: Union[PartialEmoji, str]):
        """
        ...
        return await ctx.send_success(
            f"Booster role icon successfully changed to **{emoji.name if isinstance(emoji, PartialEmoji) else emoji}**"
        )

    @boosterrole.command(name="delete", brief="server booster")
    @has_br_role()
    async def br_delete(self, ctx: GreedContext):
        """
    ...
    async def br_list(self, ctx: GreedContext):
        """
        ...
        shared_results = await self.bot.db.fetch(
            "SELECT * FROM shared_roles WHERE guild_id = $1", ctx.guild.id
        )

        entries = []
        for result in results:
            role_owner = f"<@&{result['role_id']}> owned by <@!{result['user_id']}>"
            shared_members = [
                f"<@!{shared_result['user_id']}>"
                for shared_result in shared_results
                if shared_result["role_id"] == result["role_id"]
            ]
            if shared_members:
                role_owner += f"\n shared with {', '.join(shared_members)}"
            entries.append(role_owner)
    ...
    @boosterrole.command(name="revoke")
    @has_br_role()
    @sharing_enabled()
    async def br_revoke(self, ctx: GreedContext, member: Member):
        """
        Remove a shared booster role from another user
        """
        booster_role_id = await self.bot.db.fetchval(
            "SELECT role_id FROM booster_roles WHERE guild_id = $1 AND user_id = $2",
            ctx.guild.id,
            ctx.author.id,
        )
        if not booster_role_id:
            return await ctx.send_warning("You don't have a booster role set")
        booster_role = ctx.guild.get_role(booster_role_id)
        shared_member = await self.bot.db.fetchrow(
            "SELECT user_id, owner_id FROM shared_roles WHERE guild_id = $1 AND role_id = $2 AND user_id = $3",
            ctx.guild.id,
            booster_role.id,
            member.id,
        )
        if not shared_member:
            return await ctx.send_warning(f"{member.mention} does not have this role")
        if ctx.author.id != shared_member["owner_id"]:
            owner = ctx.guild.get_member(shared_member["owner_id"])
            return await ctx.send_warning(f"Only {owner.mention}, can remove this role")
        await member.remove_roles(booster_role, reason="Booster role removed")
        await self.bot.db.execute(
            "DELETE FROM shared_roles WHERE guild_id = $1 AND role_id = $2 AND user_id = $3",
            ctx.guild.id,
            booster_role.id,
            member.id,
        )
        await ctx.send_success(f"Removed the booster role from {member.mention}")

    @boosterrole.command(name="toggle")
    @has_guild_permissions(manage_guild=True)
    async def br_toggle(self, ctx: GreedContext):
        """
        Toggle on and off the booster role share cuh
        """
        sharing_enabled = await self.bot.db.fetchval(
            "SELECT sharing_enabled FROM booster_module WHERE guild_id = $1",
            ctx.guild.id,
        )
        new_sharing_enabled = not sharing_enabled
        await self.bot.db.execute(
            "UPDATE booster_module SET sharing_enabled = $1 WHERE guild_id = $2",
            new_sharing_enabled,
            ctx.guild.id,
        )
        if new_sharing_enabled:
            await ctx.send_success("booster role can now be shared")
        else:
            await ctx.send_success("booster roles cannot be shared mf")

    @boosterrole.command(name="share")
    @has_br_role()
    @sharing_enabled()
    async def br_share(self, ctx: GreedContext, member: Member):
        """
        Share a booster role with another user
        """
        if ctx.guild.premium_subscriber_role not in ctx.author.roles:
            return await ctx.send_warning("You are not boosting the guild")
        booster_role_id = await self.bot.db.fetchval(
            "SELECT role_id FROM booster_roles WHERE guild_id = $1 AND user_id = $2",
            ctx.guild.id,
            ctx.author.id,
        ...
        if not booster_role_id:
            return await ctx.send_warning("You don't have a booster role set")
        booster_role = ctx.guild.get_role(booster_role_id)
        shared_members = await self.bot.db.fetch(
            "SELECT user_id FROM shared_roles WHERE guild_id = $1 AND role_id = $2",
            ctx.guild.id,
            booster_role.id,
        )
        if len(shared_members) >= 3:
            return await ctx.send_warning(
                "You have already shared this role with 3 members"
            )
        if booster_role in member.roles:
            return await ctx.send_warning(f"{member.mention} already has this role")
        await member.add_roles(booster_role, reason="Booster role shared")
        await self.bot.db.execute(
            "INSERT INTO shared_roles (guild_id, role_id, user_id, owner_id) VALUES ($1, $2, $3, $4)",
            ctx.guild.id,
            booster_role.id,
            member.id,
            ctx.author.id,
        )
        await ctx.send_success(f"Shared the booster role with {member.mention}")
