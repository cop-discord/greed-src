...
class Automod(Cog):
    ...
    async def whitelisted_antispam(self, message: Message):
        ...
        if res["users"]:
            users = orjson.loads(res["users"])
            if message.author.id in users:
                return True

        if res["channels"]:
            channels = orjson.loads(res["channels"])
    ...
    async def filter_joins_enable(self, ctx: GreedContext):
    ...
    @has_guild_permissions(administrator=True)
    async def filter_joins_disable(self, ctx: GreedContext):
    ...
    @has_guild_permissions(administrator=True)
    async def filter_joins_rate(self, ctx: GreedContext, rate: int):
    ...
    async def chat_filter_spam_enable(self, ctx: GreedContext):
    ...
    @has_guild_permissions(manage_guild=True)
    @antispam_enabled()
    async def chat_filter_spam_disable(self, ctx: GreedContext):
    ...
    @has_guild_permissions(manage_guild=True)
    @antispam_enabled()
    async def chat_filter_spam_rate(self, ctx: GreedContext, rate: int):
    ...
    @has_guild_permissions(manage_guild=True)
    @antispam_enabled()
    async def chat_filter_spam_timeout(self, ctx: GreedContext, time: ValidTime):
    ...
    @antispam_enabled()
    async def chat_filter_spam_settings(self, ctx: GreedContext):
        ...
        if not check["users"]:
            ...
            whitelisted_users = (
                ", ".join(list(map(lambda m: f"<@{m}>", orjson.loads(check["users"]))))
                if len(orjson.loads(check["users"])) < 5
                else ", ".join(
                    list(map(lambda m: f"<@{m}>", orjson.loads(check["users"])[:5]))
                )
        ...
        if not check["channels"]:
            ...
            whitelisted_channels = (
                ", ".join(
                    list(map(lambda m: f"<@{m}>", orjson.loads(check["channels"])))
                )
                if len(orjson.loads(check["channels"])) < 5
                else ", ".join(
                    list(map(lambda m: f"<@{m}>", orjson.loads(check["channels"])[:5]))
                )
    ...
    async def chat_filter_spam_unwhitelist(
        self, ctx: GreedContext, type: str, *, target: str
    ):
    ...
    async def chat_filter_spam_uwl_user(self, ctx: GreedContext, member: NoStaff):
        """unwhitelist an user from antispam"""
        check = await self.bot.db.fetchval(
            "SELECT users FROM antispam WHERE guild_id = $1", ctx.guild.id
        )
        if check:
            check = orjson.loads(check)
    ...
    async def chat_filter_spam_uwl_channel(
        self, ctx: GreedContext, channel: TextChannel
    ):
        """unwhitelist an user from antispam"""
        check = await self.bot.db.fetchval(
    ...
    async def chat_filter_spam_wl_user(self, ctx: GreedContext, member: NoStaff):
    ...
    async def chat_filter_invites_enable(self, ctx: GreedContext):
        """
    ...
    @has_guild_permissions(manage_guild=True)
    @bot_has_guild_permissions(manage_guild=True)
    async def chat_filter_invites_disable(self, ctx: GreedContext):
    ...
    async def chat_filter_invites_whitelisted(self, ctx: GreedContext):
    ...
    @has_guild_permissions(manage_guild=True)
    @bot_has_guild_permissions(manage_guild=True)
    async def chat_filter_words_add(self, ctx: GreedContext, *, word: str):
    ...
    @has_guild_permissions(manage_guild=True)
    @bot_has_guild_permissions(manage_guild=True)
    async def chat_filter_words_remove(self, ctx: GreedContext, *, word: str):
    ...
    @has_guild_permissions(manage_guild=True)
    @bot_has_guild_permissions(manage_guild=True)
    async def chat_filter_words_clear(self, ctx: GreedContext):
    ...
    async def chat_filter_words_list(self, ctx: GreedContext):
    ...
    async def chat_filter_words_whitelisted(self, ctx: GreedContext):
