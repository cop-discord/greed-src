...
class DeleteTicket(discord.ui.Button):
    ...
    async def make_transcript(
        self, interaction: discord.Interaction, c: discord.TextChannel
    ):
        user_id = await interaction.client.db.fetchval(
            """SELECT user_id FROM opened_tickets WHERE channel_id = $1""", c.id
        )
        user = c.guild.get_member(user_id)
        author = TicketAuthor(
            **{
                "id": str(user.name),
        ...
        return await interaction.client.tickets.upload(c, author)
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        async def yes_callback(inter: discord.Interaction) -> None:
            ...
            if check:
                ...
                if channel:
                    file = await self.make_transcript(inter, inter.channel)
                    e = discord.Embed(
                        color=inter.client.color,
                        title=f"Logs for {interaction.channel.name} `{interaction.channel.id}`",
                        url=file,
